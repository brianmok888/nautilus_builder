...
export function StrategyDetailClient({ strategyId }: { strategyId: string }) {
  ...
  useEffect(() => {
    fetchStrategyDetail(strategyId).then(setDetail).catch(() => setError("Unable to load strategy detail."));
  }, [strategyId]);
  ...
  return (
    <section>
      ...
      <ul>{detail.versions.map((version) => <li key={version.strategy_version_id}>{version.strategy_version_id}</li>)}</ul>
