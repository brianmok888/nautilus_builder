...
import { addStrategyBlock, graphToStrategySpec, updateStrategyBlockParams, type StrategyGraphState } from "../../lib/strategySpec";
...
export const StrategyBuilderWorkspace = () => {
  const [graph, setGraph] = useState<StrategyGraphState>({ nodes: [], edges: [] });
  ...
  return (
    <section>
      ...
      <MarketProfilePanel />
      <BlockPalette onAddBlock={(block) => setGraph((current) => addStrategyBlock(current, block))} />
      ...
      <BlockInspector
        ...
        onChangeParams={(params) => {
          if (selectedNode) {
            setGraph((current) => updateStrategyBlockParams(current, selectedNode.id, params));
          }
        }}
      />
