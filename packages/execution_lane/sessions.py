...
def _client_configs(
    *,
    ...
    if venue == "BINANCE":
        return _binance_client_configs(profile=profile, command=command, credential_values=credential_values)
...
def _binance_client_configs(
    *,
    ...
    api_key = _credential(credential_values, "BINANCE_API_KEY")
    api_secret = _credential(credential_values, "BINANCE_API_SECRET")
    ...
    account_type = BinanceAccountType.USDT_FUTURES if any(token in adapter_hint for token in ("PERP", "FUTURE", "FUTURES")) else BinanceAccountType.SPOT
